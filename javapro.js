...
const TG_NOTIFIED_CACHE = new Map(); // username -> "YYYY-MM-DD" of last alert sent, avoids duplicate Telegram spam
const TG_SETTINGS_CACHE = { token: null, chatId: null, fetchedAt: 0 };
...
export default {
	async fetch(request, env, ctx) {
		...
		if (Router.isWebSocketUpgrade(request) && url.pathname === "/JavaPro_Secure_Tunnel") {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				...
				const metadata = {
					main_module: "javapro.js",
					compatibility_date: "2024-02-08",
					bindings: newBindings,
				};
				const formData = new FormData();
				formData.append("metadata", new Blob([JSON.stringify(metadata)], { type: "application/json" }));
				formData.append("javapro.js", new Blob([newCode], { type: "application/javascript+module" }), "javapro.js");
		...
		if (url.pathname === "/api/proxy-ip") {
			if (request.method === "POST") {
				const { proxy_ip, iata, frag_len, frag_int, tg_bot_token, tg_chat_id } = await request.json();
				...
				if (tg_bot_token !== undefined) await env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('tg_bot_token', ?)").bind(tg_bot_token).run();
				if (tg_chat_id !== undefined) await env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('tg_chat_id', ?)").bind(tg_chat_id).run();
				return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
			}
			if (request.method === "GET") {
				...
				const rowTgToken = await env.DB.prepare("SELECT value FROM settings WHERE key = 'tg_bot_token'").first();
				const rowTgChat = await env.DB.prepare("SELECT value FROM settings WHERE key = 'tg_chat_id'").first();
				return new Response(
					JSON.stringify({
						...
						tg_bot_token: rowTgToken ? rowTgToken.value : "",
						tg_chat_id: rowTgChat ? rowTgChat.value : "",
					}),
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2FJavaPro_Secure_Tunnel#" + encodeURIComponent(m1));
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2FJavaPro_Secure_Tunnel#" + encodeURIComponent(m2));
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + host + ":80?path=%2FJavaPro_Secure_Tunnel&security=none&encryption=none&host=" + host + "&fp=" + fp + "&type=ws#" + encodeURIComponent(infoRemark));
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				const isTlsPort = ["443", "2053", "2083", "2087", "2096", "8443"].includes(portStr);
				const tlsVal = isTlsPort ? "tls" : "none";
				const remark = user.username + " | " + ip + " | " + portStr;
				links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + ip + ":" + portStr + "?path=%2FJavaPro_Secure_Tunnel&security=" + tlsVal + "&encryption=none&insecure=0&host=" + host + "&fp=" + fp + "&type=ws&allowInsecure=0&sni=" + host + "#" + encodeURIComponent(remark));
			});
		});
...
async function getTelegramSettings(env) {
	const now = Date.now();
	if (TG_SETTINGS_CACHE.token !== null && now - TG_SETTINGS_CACHE.fetchedAt < 5 * 60 * 1000) {
		return { token: TG_SETTINGS_CACHE.token, chatId: TG_SETTINGS_CACHE.chatId };
	}
	try {
		const rowToken = await env.DB.prepare("SELECT value FROM settings WHERE key = 'tg_bot_token'").first();
		const rowChat = await env.DB.prepare("SELECT value FROM settings WHERE key = 'tg_chat_id'").first();
		TG_SETTINGS_CACHE.token = (rowToken && rowToken.value) || "";
		TG_SETTINGS_CACHE.chatId = (rowChat && rowChat.value) || "";
		TG_SETTINGS_CACHE.fetchedAt = now;
	} catch (e) {
		TG_SETTINGS_CACHE.token = "";
		TG_SETTINGS_CACHE.chatId = "";
	}
	return { token: TG_SETTINGS_CACHE.token, chatId: TG_SETTINGS_CACHE.chatId };
}
async function sendTelegramAlert(env, text) {
	try {
		const { token, chatId } = await getTelegramSettings(env);
...
async function checkAndNotifyUsage(env, ctx, username) {
	try {
		const { token, chatId } = await getTelegramSettings(env);
		if (!token || !chatId) return;
		const todayStr = new Date().toISOString().slice(0, 10);
		if (TG_NOTIFIED_CACHE.get(username) === todayStr) return;
		const user = await env.DB.prepare("SELECT limit_gb, used_gb, limit_req, used_req FROM users WHERE username = ?").bind(username).first();
		if (!user) return;
		let reasons = [];
		if (user.limit_gb && user.used_gb >= user.limit_gb * 0.9) {
			reasons.push(`حجم مصرفی: ${user.used_gb.toFixed(2)}GB از ${user.limit_gb}GB`);
		}
		if (user.limit_req && user.used_req >= user.limit_req * 0.9) {
			reasons.push(`تعداد ریکوئست: ${user.used_req} از ${user.limit_req}`);
		}
		if (reasons.length === 0) return;
		TG_NOTIFIED_CACHE.set(username, todayStr);
		const text = `⚠️ <b>هشدار نزدیک شدن به سقف مصرف</b>\nکاربر: <code>${username}</code>\n${reasons.join("\n")}`;
		const task = sendTelegramAlert(env, text);
		if (ctx) ctx.waitUntil(task);
		else await task;
	} catch (e) {
		console.error("checkAndNotifyUsage failed:", e.message);
	}
}
		...
		if (current >= thresholdBytes || (current > 0 && now - lastDbWrite > 60000)) {
			...
			let writeTask = async () => {
				try {
					...
					await checkAndNotifyUsage(env, ctx, username);
				} catch (e) {
					console.error(e.message);
				} finally {
					GLOBAL_WRITE_LOCK.set(username, false);
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://github.com/parsasanat/java-pro" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="GitHub">
        ...
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-zinc-800">
                    <h4 class="text-sm font-bold mb-3 text-gray-800 dark:text-zinc-200">🔔 اعلان تلگرام (اتمام حجم/ریکوئست کاربران)</h4>
                    <div class="space-y-3">
                        <div>
                            <label class="block text-[11px] text-gray-500 dark:text-gray-400 font-medium mb-1">Bot Token</label>
                            <input type="text" id="tg-bot-token" placeholder="123456:ABC-DEF..." class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs font-mono text-center" dir="ltr">
                        </div>
                        <div>
                            <label class="block text-[11px] text-gray-500 dark:text-gray-400 font-medium mb-1">Chat ID (آیدی عددی شما یا گروه)</label>
                            <input type="text" id="tg-chat-id" placeholder="-1001234567890" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs font-mono text-center" dir="ltr">
                        </div>
                        <p class="text-[10px] text-gray-400 dark:text-zinc-500 leading-relaxed">وقتی مصرف حجم یا ریکوئست کاربری به ۹۰٪ سقف تعیین‌شده برسه، یک بار در روز پیام هشدار به این چت ارسال می‌شود.</p>
                    </div>
                </div>
        ...
        function renderUsersUI(data) {
            try {
                ...
                if (cfRequests >= 90000) {
                    ...
                    const today = new Date().toISOString().split('T')[0];
                    if (localStorage.getItem('javapro_usage_warned_date') !== today) {
                        openUsageWarning();
                    }
                } else {
                    if (reqCard) {
...
function closePathWarning() {
    ...
    card.classList.remove('opacity-100', 'scale-100');
    card.classList.add('opacity-0', 'scale-95');
    localStorage.setItem('javapro_path_warned_' + CURRENT_VERSION, 'true');
}
function closeUsageWarning() {
    ...
    card.classList.add('opacity-0', 'scale-95');
    const today = new Date().toISOString().split('T')[0];
    localStorage.setItem('javapro_usage_warned_date', today);
}
function openUsageWarning() {
    ...
    card.classList.add('opacity-100', 'scale-100');
        ...
        function getVlessLink(username) {
            ...
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FJavaPro_Secure_Tunnel#' + encodeURIComponent(m1));
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FJavaPro_Secure_Tunnel#' + encodeURIComponent(m2));
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    const isTlsPort = tlsPorts.includes(portStr);
                    const tlsVal = isTlsPort ? 'tls' : 'none';
                    const remark = user.username + ' | ' + ip + ' | ' + portStr;
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FJavaPro_Secure_Tunnel&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
...
function editUser(encodedUsername) {
    ...
    toggleModal(true);
        ...
        async function loadLocations() {
            ...
            try {
                ...
                if (statusRes.ok) {
                    ...
                    document.getElementById('tg-bot-token').value = statusData.tg_bot_token || '';
                    document.getElementById('tg-chat-id').value = statusData.tg_chat_id || '';
                }
        ...
        async function saveSettings() {
            ...
            const tgBotToken = document.getElementById('tg-bot-token').value.trim();
            ...
            try {
                ...
                const response = await fetch('/api/proxy-ip', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ proxy_ip: resolvedIp, iata: iata ? iata.toUpperCase() : '', frag_len: fragLen, frag_int: fragInt, tg_bot_token: tgBotToken, tg_chat_id: tgChatId })
                });
                if (response.ok) {
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/parsasanat/java-pro/refs/heads/main/javapro.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/parsasanat/java-pro/refs/heads/main/ips.txt');
...
document.addEventListener('DOMContentLoaded', () => {
        if (localStorage.getItem('javapro_path_warned_' + CURRENT_VERSION) !== 'true') {
            const modal = document.getElementById('path-warning-modal');
            const card = modal.querySelector('div');
            modal.classList.remove('opacity-0', 'pointer-events-none');
            modal.classList.add('opacity-100', 'pointer-events-auto');
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل جاوا-پرو - وضعیت اشتراک</h1>
...
<div class="flex items-center gap-4 mt-6 z-10">
    <a href="https://github.com/parsasanat/java-pro" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FJavaPro_Secure_Tunnel&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
